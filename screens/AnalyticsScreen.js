...
import { categoryNames } from '../constants/globalConstants';
...
export default function AnalyticsScreen() {
      ...
      useEffect(() => {
        if (startDate && endDate && transactions.length > 0) {
            ...
            const filteredTransactions = transactions.filter((transaction) => {
                return transaction.date >= startTimestamp && transaction.date <= endTimestamp && transaction.type === 'expense';
            });
    ...
    return (
        <Layout>
            <View style={{backgroundColor: '#FFFFFF', borderRadius: 20, marginVertical: 20, borderWidth: 1}}>
